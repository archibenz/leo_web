...
export default function WhiteShopShowcase({locale, initialCat = 'all', initialQuery = '', initialSort = 'new', focusSearch = false}: {locale: string; initialCat?: Cat | 'all'; initialQuery?: string; initialSort?: Sort; focusSearch?: boolean}) {
  ...
  const clearFilters = () => {
    ...
    setQuery('');
    setSort('new');
    if (typeof window !== 'undefined') window.history.replaceState(null, '', `/${locale}/shop`);
  };
  ...
  return (
    <>
      ...
      <main id="wv-main" tabIndex={-1} style={{outline: 'none'}} className="mx-auto max-w-[1400px] sm:px-10">
        ...
        {/* Grid. Two columns on a phone as well: at one column the eighteen
            pieces ran to twenty screens of scrolling, and a shopper cannot
            compare two garments they can never see at once. `bleed` is dropped
            below sm — it exists to run a single column edge to edge, and with
