...
// Enter/exit transition duration — keep in sync with the duration-200 classes
// on the backdrop + panel below.
...
export default function DeleteAccountModal({open, onClose, onConfirm, onRequestChallenge, hasPassword}: Props) {
  ...
  const {mounted, entered} = useMountTransition(open, TRANSITION_MS);
  ...
  return createPortal(
    <>
      <div
        className={`fixed inset-0 z-[300] bg-black/55 backdrop-blur-sm transition-opacity duration-200 ease-out motion-reduce:transition-none ${
      ...
      <div
        className={`fixed inset-0 z-[301] flex items-center justify-center p-4 transition duration-200 ease-out motion-reduce:transition-none ${
          entered ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-[0.92] translate-y-5'
        }`}
        role="dialog"
        aria-modal="true"
