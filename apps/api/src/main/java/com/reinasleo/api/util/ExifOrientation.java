 ...
 * → IFD0 → тег 0x0112. И 'II' (little-endian), и 'MM' (big-endian) читаются
 * правильно — так что «чужой порядок байт» ниже не про MM, единственная
 * альтернатива 'II' по спецификации TIFF: он читается ничуть не хуже. Всё,
 * что не сошлось (нет сегмента, порядок байт не 'II' и не 'MM', обрезанный
 * файл), читается как «поворота нет» — это безопасная сторона ошибки: снимок
 * останется как есть, а не перевернётся наугад.
 */
